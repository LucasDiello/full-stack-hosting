...
export const getAllPosts = async (req, res) => {
  ...
  try {
    const { status, posts, pagination } = await serviceGetAllPosts(query);
    res.status(mapStatusHTTP(status)).json({ posts, pagination });
  } catch (error) {
    res
      .status(mapStatusHTTP("INTERNAL_SERVER_ERROR"))
      .json({ message: "Error retrieving posts" });
  }
};
...
export const getPostById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(mapStatusHTTP("INTERNAL_SERVER_ERROR"))
      .json({ message: "Error retrieving post" });
  }
};
